object FCitas: TFCitas
  Left = 164
  Top = 161
  AutoScroll = False
  BorderIcons = []
  Caption = 'Citas '#166' Crear Nueva'
  ClientHeight = 317
  ClientWidth = 816
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object bsSkinBevel2: TbsSkinBevel
    Left = 496
    Top = 203
    Width = 278
    Height = 94
    Shape = bsFrame
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'bevel'
    DividerMode = False
  end
  object LB_1: TbsSkinStdLabel
    Left = 32
    Top = 184
    Width = 49
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'Hora Cita'
  end
  object LB_2: TbsSkinStdLabel
    Left = 240
    Top = 187
    Width = 90
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'Motivo de la Cita'
  end
  object LB_3: TbsSkinStdLabel
    Left = 32
    Top = 225
    Width = 101
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'Tiempo Reservado'
  end
  object bsSkinBevel1: TbsSkinBevel
    Left = 240
    Top = 248
    Width = 249
    Height = 49
    Shape = bsFrame
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'bevel'
    DividerMode = False
  end
  object LB_4: TbsSkinStdLabel
    Left = 240
    Top = 4
    Width = 31
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -13
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = [fsBold]
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'LB_4'
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    Visible = False
  end
  object LB_5: TbsSkinStdLabel
    Left = 506
    Top = 207
    Width = 68
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'Hora inicio:'
    ParentFont = False
  end
  object LB_6: TbsSkinStdLabel
    Left = 506
    Top = 224
    Width = 117
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'Duracion estimada:'
    ParentFont = False
  end
  object LB_7: TbsSkinStdLabel
    Left = 506
    Top = 242
    Width = 79
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'Hora finaliza:'
    ParentFont = False
  end
  object LB_8: TbsSkinStdLabel
    Left = 506
    Top = 259
    Width = 107
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'Nombre paciente:'
    ParentFont = False
  end
  object LB_13: TbsSkinStdLabel
    Left = 506
    Top = 277
    Width = 75
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = [fsBold]
    SkinDataName = 'stdlabel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Caption = 'Motivo Cita:'
    ParentFont = False
  end
  object LB_9: TbsSkinStdLabel
    Left = 579
    Top = 207
    Width = 27
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'LB_9'
    Visible = False
  end
  object LB_10: TbsSkinStdLabel
    Left = 626
    Top = 225
    Width = 34
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'LB_10'
    Visible = False
  end
  object LB_11: TbsSkinStdLabel
    Left = 590
    Top = 243
    Width = 34
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'LB_11'
    Visible = False
  end
  object LB_12: TbsSkinStdLabel
    Left = 618
    Top = 260
    Width = 34
    Height = 14
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    Caption = 'LB_12'
    Visible = False
  end
  object LB_14: TbsSkinStdLabel
    Left = 585
    Top = 277
    Width = 179
    Height = 13
    EllipsType = bsetNone
    UseSkinFont = False
    UseSkinColor = True
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'MS Sans Serif'
    DefaultFont.Style = []
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'stdlabel'
    AutoSize = False
    Caption = 'LB_14'
    Visible = False
  end
  object Calendario_1: TbsSkinMonthCalendar
    Left = 32
    Top = 25
    Width = 200
    Height = 159
    Hint = 'Seleccione una fecha @ -Superior a la actual. @ -O igual'
    HintTitle = 'Fecha de Cita'
    HintImageIndex = 0
    HintImageList = ImgList_Citas
    TabOrder = 0
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'panel'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = False
    RibbonStyle = False
    ImagePosition = bsipDefault
    TransparentMode = False
    CaptionImageIndex = -1
    RealHeight = -1
    AutoEnabledControls = True
    CheckedMode = False
    Checked = False
    DefaultAlignment = taLeftJustify
    DefaultCaptionHeight = 22
    BorderStyle = bvFrame
    CaptionMode = False
    RollUpMode = False
    RollUpState = False
    NumGlyphs = 1
    Spacing = 2
    Caption = 'Calendario_1'
    ParentShowHint = False
    ShowHint = True
    Date = 40442.314954780100000000
    TodayDefault = True
    FirstDayOfWeek = Sun
    OnNumberClick = Calendario_1NumberClick
    BoldDays = False
  end
  object DBGRID_Citas: TbsSkinDBGrid
    Left = 240
    Top = 24
    Width = 513
    Height = 137
    HintImageIndex = 0
    TabOrder = 1
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'grid'
    Transparent = True
    WallpaperStretch = False
    UseSkinFont = False
    UseSkinCellHeight = True
    VScrollBar = bsSkinScrollBar1
    GridLineColor = clWindowText
    DefaultCellHeight = 20
    DrawGraphicFields = False
    UseColumnsFont = False
    DefaultRowHeight = 18
    MouseWheelSupport = False
    SaveMultiSelection = False
    PickListBoxSkinDataName = 'listbox'
    PickListBoxCaptionMode = False
    DataSource = DS_Citas
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection]
    PopupMenu = PPM_OpcionesCita
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -12
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnCellClick = DBGRID_CitasCellClick
    OnDblClick = DBGRID_CitasDblClick
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'ID_RECORDS_CITA'
        Title.Alignment = taCenter
        Title.Caption = 'Records Paciente'
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOMBRE_PACIENTE'
        Title.Alignment = taCenter
        Title.Caption = 'Nombre Paciente'
        Width = 151
        Visible = True
      end
      item
        Expanded = False
        FieldName = '_HORA'
        Title.Alignment = taCenter
        Title.Caption = 'Fecha/Hora'
        Width = 171
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'DURACION_CITA'
        Title.Alignment = taCenter
        Title.Caption = 'Duraci'#243'n'
        Width = 73
        Visible = True
      end>
  end
  object M_1: TbsSkinMemo
    Left = 240
    Top = 203
    Width = 249
    Height = 41
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = 14
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ScrollBars = ssVertical
    TabOrder = 2
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clBlack
    DefaultFont.Height = 14
    DefaultFont.Name = 'Arial'
    DefaultFont.Style = []
    UseSkinFont = False
    UseSkinFontColor = True
    BitMapBG = True
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'memo'
  end
  object DateTimePicker1: TDateTimePicker
    Left = 32
    Top = 200
    Width = 153
    Height = 21
    Date = 40385.333333333340000000
    Time = 40385.333333333340000000
    Enabled = False
    Kind = dtkTime
    TabOrder = 3
  end
  object BT_GuardaCita: TbsSkinButton
    Left = 246
    Top = 256
    Width = 75
    Height = 25
    HintImageIndex = 0
    TabOrder = 4
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'button'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = False
    ImageIndex = -1
    AlwaysShowLayeredFrame = False
    UseSkinSize = False
    UseSkinFontColor = True
    RepeatMode = False
    RepeatInterval = 100
    AllowAllUp = False
    TabStop = True
    CanFocused = True
    Down = False
    GroupIndex = 0
    Caption = 'Aceptar'
    NumGlyphs = 1
    Spacing = 1
    OnClick = BT_GuardaCitaClick
  end
  object BT_Limpiar: TbsSkinButton
    Left = 326
    Top = 256
    Width = 75
    Height = 25
    HintImageIndex = 0
    TabOrder = 5
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'button'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = False
    ImageIndex = -1
    AlwaysShowLayeredFrame = False
    UseSkinSize = False
    UseSkinFontColor = True
    RepeatMode = False
    RepeatInterval = 100
    AllowAllUp = False
    TabStop = True
    CanFocused = True
    Down = False
    GroupIndex = 0
    Caption = 'Limpiar'
    NumGlyphs = 1
    Spacing = 1
    OnClick = BT_LimpiarClick
  end
  object BT_Retornar: TbsSkinButton
    Left = 406
    Top = 256
    Width = 75
    Height = 25
    HintImageIndex = 0
    TabOrder = 6
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'button'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = False
    ImageIndex = -1
    AlwaysShowLayeredFrame = False
    UseSkinSize = False
    UseSkinFontColor = True
    RepeatMode = False
    RepeatInterval = 100
    AllowAllUp = False
    TabStop = True
    CanFocused = True
    Down = False
    GroupIndex = 0
    Caption = 'Retornar'
    NumGlyphs = 1
    Spacing = 1
    OnClick = BT_RetornarClick
  end
  object bsSkinScrollBar1: TbsSkinScrollBar
    Left = 750
    Top = 23
    Width = 19
    Height = 161
    HintImageIndex = 0
    TabOrder = 7
    Visible = False
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'vscrollbar'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = 14
    DefaultFont.Name = 'Arial'
    DefaultFont.Style = []
    DefaultWidth = 19
    DefaultHeight = 0
    UseSkinFont = True
    Both = False
    BothMarkerWidth = 19
    BothSkinDataName = 'bothhscrollbar'
    CanFocused = False
    Kind = sbVertical
    PageSize = 0
    Min = 0
    Max = 100
    Position = 0
    SmallChange = 1
    LargeChange = 1
  end
  object bsSkinDBNavigator1: TbsSkinDBNavigator
    Left = 241
    Top = 166
    Width = 510
    Height = 18
    HintImageIndex = 0
    TabOrder = 8
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'panel'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = True
    RibbonStyle = False
    ImagePosition = bsipDefault
    TransparentMode = False
    CaptionImageIndex = -1
    RealHeight = -1
    AutoEnabledControls = True
    CheckedMode = False
    Checked = False
    DefaultAlignment = taLeftJustify
    DefaultCaptionHeight = 22
    BorderStyle = bvNone
    CaptionMode = False
    RollUpMode = False
    RollUpState = False
    NumGlyphs = 1
    Spacing = 2
    AdditionalGlyphs = False
    DataSource = DS_Citas
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbRefresh]
    BtnSkinDataName = 'button'
  end
  object SEdit_DuracionCita: TbsSkinSpinEdit
    Left = 33
    Top = 240
    Width = 105
    Height = 20
    HintImageIndex = 0
    TabOrder = 9
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'spinedit'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = 14
    DefaultFont.Name = 'Arial'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = True
    DefaultColor = clWindow
    UseSkinSize = False
    Enabled = False
    ValueType = vtInteger
    MinValue = 5.000000000000000000
    MaxValue = 1460.000000000000000000
    Increment = 5.000000000000000000
    EditorEnabled = True
    MaxLength = 0
  end
  object RadioCheck_1: TbsSkinCheckRadioBox
    Left = 32
    Top = 264
    Width = 201
    Height = 25
    HintImageIndex = 0
    TabOrder = 10
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    SkinDataName = 'checkbox'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultWidth = 0
    DefaultHeight = 0
    UseSkinFont = False
    WordWrap = False
    AllowGrayed = False
    State = cbUnchecked
    ImageIndex = 0
    Flat = True
    UseSkinFontColor = True
    TabStop = True
    CanFocused = True
    Radio = False
    Checked = False
    GroupIndex = 0
    Caption = 'Usar control de tiempo en citas'
    OnClick = RadioCheck_1Click
  end
  object BSSF_Citas: TbsBusinessSkinForm
    QuickButtons = <>
    QuickButtonsShowHint = False
    ClientInActiveEffect = False
    ClientInActiveEffectType = bsieSemiTransparent
    DisableSystemMenu = False
    AlwaysResize = False
    PositionInMonitor = bspDefault
    UseFormCursorInNCArea = False
    MaxMenuItemsInWindow = 0
    ClientWidth = 0
    ClientHeight = 0
    HideCaptionButtons = False
    AlwaysShowInTray = False
    LogoBitMapTransparent = False
    AlwaysMinimizeToTray = False
    UseSkinFontInMenu = True
    ShowIcon = False
    MaximizeOnFullScreen = False
    AlphaBlend = False
    AlphaBlendAnimation = False
    AlphaBlendValue = 200
    ShowObjectHint = False
    MenusAlphaBlend = False
    MenusAlphaBlendAnimation = False
    MenusAlphaBlendValue = 200
    DefCaptionFont.Charset = DEFAULT_CHARSET
    DefCaptionFont.Color = clBtnText
    DefCaptionFont.Height = 14
    DefCaptionFont.Name = 'Arial'
    DefCaptionFont.Style = [fsBold]
    DefInActiveCaptionFont.Charset = DEFAULT_CHARSET
    DefInActiveCaptionFont.Color = clBtnShadow
    DefInActiveCaptionFont.Height = 14
    DefInActiveCaptionFont.Name = 'Arial'
    DefInActiveCaptionFont.Style = [fsBold]
    DefMenuItemHeight = 20
    DefMenuItemFont.Charset = DEFAULT_CHARSET
    DefMenuItemFont.Color = clWindowText
    DefMenuItemFont.Height = 14
    DefMenuItemFont.Name = 'Arial'
    DefMenuItemFont.Style = []
    UseDefaultSysMenu = True
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    MinHeight = 0
    MinWidth = 0
    MaxHeight = 0
    MaxWidth = 0
    Magnetic = False
    MagneticSize = 5
    BorderIcons = []
    Left = 432
    Top = 88
  end
  object Hins_Citas: TbsSkinHint
    LineSeparator = '@'
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    Active = True
    AlphaBlend = False
    AlphaBlendValue = 0
    AlphaBlendAnimation = False
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -11
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    UseSkinFont = True
    Left = 368
    Top = 88
  end
  object ImgList_Citas: TImageList
    Height = 32
    Width = 32
    Left = 400
    Top = 88
    Bitmap = {
      
      000000000000}
  end
  object DS_Citas: TDataSource
    DataSet = CD_BuscaCita
    Left = 256
    Top = 88
  end
  object CD_BuscaCita: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        ParamType = ptInputOutput
        Value = #39#39
      end
      item
        DataType = ftDateTime
        ParamType = ptInputOutput
      end>
    ProviderName = 'DSP_BuscaCita'
    RemoteServer = FPrincipal.SKConexion
    Left = 288
    Top = 88
    object CD_BuscaCitaNOMBRE_PACIENTE: TStringField
      FieldName = 'NOMBRE_PACIENTE'
      ReadOnly = True
      Size = 111
    end
    object CD_BuscaCita_HORA: TStringField
      FieldName = '_HORA'
      ReadOnly = True
      Size = 30
    end
    object CD_BuscaCita_FECHA: TStringField
      FieldName = '_FECHA'
      ReadOnly = True
      Size = 30
    end
    object CD_BuscaCitaIDCITA: TAutoIncField
      FieldName = 'IDCITA'
      ReadOnly = True
    end
    object CD_BuscaCitaID_MEDICO: TStringField
      FieldName = 'ID_MEDICO'
      Size = 11
    end
    object CD_BuscaCitaID_RECORDS_CITA: TIntegerField
      FieldName = 'ID_RECORDS_CITA'
    end
    object CD_BuscaCitaMOTIVO_CITA: TStringField
      FieldName = 'MOTIVO_CITA'
      Size = 50
    end
    object CD_BuscaCitaFECHA: TDateTimeField
      FieldName = 'FECHA'
    end
    object CD_BuscaCitaHORA: TDateTimeField
      FieldName = 'HORA'
    end
    object CD_BuscaCitaDURACION_CITA: TIntegerField
      FieldName = 'DURACION_CITA'
    end
    object CD_BuscaCitaHORA_AVILITADO: TStringField
      FieldName = 'HORA_AVILITADO'
      FixedChar = True
      Size = 1
    end
    object CD_BuscaCitaFECHA_CREACION_CITA: TDateTimeField
      FieldName = 'FECHA_CREACION_CITA'
    end
  end
  object CD_InsertaCita: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftDate
        ParamType = ptInput
      end>
    ProviderName = 'DSP_InsertaCita'
    RemoteServer = FPrincipal.SKConexion
    Left = 288
    Top = 209
  end
  object CD_ModificaCita: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftDate
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end
      item
        DataType = ftString
        ParamType = ptInput
      end>
    ProviderName = 'DSP_ModificaCita'
    RemoteServer = FPrincipal.SKConexion
    Left = 328
    Top = 208
  end
  object CD_EliminaCita: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        ParamType = ptInput
      end>
    ProviderName = 'DSP_EliminaCita'
    RemoteServer = FPrincipal.SKConexion
    Left = 368
    Top = 208
  end
  object Mensaje_Citas: TbsSkinMessage
    ShowAgainFlag = False
    ShowAgainFlagValue = False
    AlphaBlend = False
    AlphaBlendAnimation = False
    AlphaBlendValue = 200
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    CtrlSkinData = FPrincipal.BSDataSkin_FPrincipal
    ButtonSkinDataName = 'button'
    MessageLabelSkinDataName = 'stdlabel'
    DefaultFont.Charset = DEFAULT_CHARSET
    DefaultFont.Color = clWindowText
    DefaultFont.Height = -12
    DefaultFont.Name = 'Tahoma'
    DefaultFont.Style = []
    DefaultButtonFont.Charset = DEFAULT_CHARSET
    DefaultButtonFont.Color = clWindowText
    DefaultButtonFont.Height = 14
    DefaultButtonFont.Name = 'Arial'
    DefaultButtonFont.Style = []
    UseSkinFont = False
    Left = 464
    Top = 88
  end
  object CD_ValidaCitaBusca: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        ParamType = ptInputOutput
      end
      item
        DataType = ftDateTime
        ParamType = ptInputOutput
      end
      item
        DataType = ftString
        ParamType = ptInputOutput
        Value = #39#39
      end>
    ProviderName = 'DSP_ValidaCitaBusca'
    RemoteServer = FPrincipal.SKConexion
    Left = 320
    Top = 88
    object CD_ValidaCitaBuscaID_RECORDS_CITA: TIntegerField
      FieldName = 'ID_RECORDS_CITA'
    end
    object CD_ValidaCitaBuscaFECHA: TDateTimeField
      FieldName = 'FECHA'
    end
  end
  object CD_CitaVirtual: TClientDataSet
    Active = True
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'IdMedico'
        DataType = ftString
        Size = 13
      end
      item
        Name = 'Id_Records'
        DataType = ftString
        Size = 7
      end
      item
        Name = 'Hora'
        DataType = ftDateTime
      end
      item
        Name = 'fecha'
        DataType = ftDateTime
      end
      item
        Name = 'duracion'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'fechacreado'
        DataType = ftDateTime
      end
      item
        Name = 'motivocita'
        DataType = ftString
        Size = 60
      end>
    IndexDefs = <>
    Params = <>
    StoreDefs = True
    Left = 256
    Top = 120
    Data = {
      
      003C000000}
    object CD_CitaVirtualIdMedico: TStringField
      FieldName = 'IdMedico'
      Size = 13
    end
    object CD_CitaVirtualId_Records: TStringField
      FieldName = 'Id_Records'
      Size = 7
    end
    object CD_CitaVirtualHora: TDateTimeField
      FieldName = 'Hora'
    end
    object CD_CitaVirtualfecha: TDateTimeField
      FieldName = 'fecha'
    end
    object CD_CitaVirtualduracion: TStringField
      FieldName = 'duracion'
      Size = 10
    end
    object CD_CitaVirtualfechacreado: TDateTimeField
      FieldName = 'fechacreado'
    end
    object CD_CitaVirtualmotivocita: TStringField
      FieldName = 'motivocita'
      Size = 60
    end
  end
  object PPM_OpcionesCita: TbsSkinPopupMenu
    SkinData = FPrincipal.BSDataSkin_FPrincipal
    Left = 496
    Top = 88
    object PPMSubM_Eliminar: TMenuItem
      Caption = 'Eliminar'
      Enabled = False
      OnClick = PPMSubM_EliminarClick
    end
    object PPMSubM_N1: TMenuItem
      Caption = '-'
    end
    object PPMSubM_MoverCita: TMenuItem
      Caption = 'Mover Cita'
      Enabled = False
      OnClick = PPMSubM_MoverCitaClick
    end
  end
end
